import React from 'react'
import { Row, Col, Image, Card, Button, Table } from 'antd';
import { DeleteOutlined } from '@ant-design/icons';

const DetailSettingUser = () => {

    const tabelAkses = [
        {
            title: 'Role',
            dataIndex: 'role',
            key: 'role',
        },
        {
            title: 'Aksi',
            key: 'aksi',
            width: '30%',
            render: (_, record) => (
                <Button variant='outlined' color='danger' icon={<DeleteOutlined />} ></Button>
            )
        }
    ];

    const dataAkses = [
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 1,
            role: 'Admin',
        },
        {
            id: 2,
            role: 'Petugas Loket',
        }
    ]

    return (
        <>
            <Row>
                <Col xs={24} md={8} style={{display: 'flex', justifyContent: 'center' }}>
                <Image
                    style={{ marginBottom: '20px'}}
                    width="75%"
                    preview={false}
                    src="https://zos.alipayobjects.com/rmsportal/jkjgkEfvpUPVyRjUImniVslZfWPnJuuZ.png"
                    fallback='data:image/png;base64,'
                />
                </Col>
                <Col xs={24} md={16}>
                    <Card>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>NIP</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>199807142018121001</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Nama</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>RAHMAT DEKY SOFYAN HIDAYAT</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Golongan</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>II/b</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Jabatan</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>Pelaksana Pemeriksa</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Bidang / Bagian</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>Umum</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Email</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>rahmat.hidayat98@kemenkeu.go.id</Col>
                        </Row>
                        <Row>
                            <Col xs={9} md={6} className='data-detail-user'>Username</Col>
                            <Col xs={2} md={1} className='data-detail-user'>:</Col>
                            <Col xs={24} md={17} className='data-detail-user'>rahmat.deky</Col>
                        </Row>
                    </Card>
                    <Row style={{ margin: "20px 0 " }} gutter={[16, 16]}>
                        <Col xs={24} sm={6} md={6} lg={4}>
                            <Button color="default" variant="outlined" block size="large">
                                Tambah Role
                            </Button>
                        </Col>
                        <Col xs={24} sm={6} md={6} lg={4}>
                            <Button color="primary" variant="solid" block size="large">
                                Edit User
                            </Button>
                        </Col>
                        <Col xs={24} sm={6} md={6} lg={4}>
                            <Button color="danger" variant="solid" block size="large">
                                Ganti Password
                            </Button>
                        </Col>
                    </Row>
                    <Row>
                        <Col lg={12}>
                            <Table columns={tabelAkses} dataSource={dataAkses} pagination={false} scroll={{ y: 300 }} />
                        </Col>
                    </Row>
                </Col>
            </Row>
        </>
    )
}

export default DetailSettingUser;
